...
do
	...
	if [ $num -lt 1 ]; then
        ...
        /usr/bin/python /usr/local/bin/celery worker -A periodic_tasks -s /tmp/celery_scheduler -Q periodic --concurrency 2 --loglevel=DEBUG --pid /tmp/periodic_tasks.pid --beat &

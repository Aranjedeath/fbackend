...
def get_discover_list(current_user_id, offset, limit=10, day_count=0, add_super=False, exclude_users=[]):
    ...
    all_other_items = make_resp_multitype(current_user_id, resp,
                                          ...
                                          reverse_sort=True)
    all_other_items = [all_other_items.insert(dirty_item['dirty_index'],
                                              dirty_item
                                             ) for dirty_item in dirty_items]
    all_other_items = [all_other_items.insert(recycled_item['recycled_index'],
                                              recycled_item
    ...
    items = db.session.execute(
        text("""SELECT
                ...
                WHERE 
                    ...
                    AND discover_list.id <= :end_pointer
                ORDER BY id DESC
        """),
        params={'start_pointer':recycled_upto,
                'end_pointer':recycled_upto + config.DISCOVER_RECYCLE_COUNT
               }
        )
    resp = {}
...
def check_user_last_visit_threshold_cross(last_visit):
    return datetime.datetime.now() - last_visit > config.DISCOVER_RECYCLE_TIME

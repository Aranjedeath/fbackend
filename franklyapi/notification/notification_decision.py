...
def decide_question_push(user_id, question_id):
    ...
    if notifications_sent_today < 2 and good_time:
        ...
        if questions_today > 10 or is_popular(user_id):
            ...
            upvotes = user_stats.get_question_upvote_count(question_id)
...
def prompt_sharing_popular_question():
    ...
    for row in results:
        ...
        if upvote_count > 10:
           upvote_count = user_stats.get_question_upvote_count(row[4])
...
def user_followers_milestone_notifications():
    ...
    for row in result:
        check_and_make_milestone('user_followers_milestone', row[0], row[0], user_stats.get_follower_count(row[0]))
...
def decide_post_milestone(post_id, user_id):
    ...
    check_and_make_milestone('post-likes-milestone', user_id, post_id, user_stats.get_post_like_count(post_id))

def decide_follow_milestone(user_id):
    check_and_make_milestone('user-followers-milestone', user_id, user_id, user_stats.get_follower_count(user_id))
...
def question_upvotes_milestone_notifications():
    ...
    for row in result:
        check_and_make_milestone('post_likes', row[1], row[0], user_stats.get_post_like_count(row[0]))

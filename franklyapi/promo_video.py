...
def overlay(bg_file, ov_bg_file, ov_static_file, ov_black_line_file,
    ...
    if text and text_font and text_color:
        ...
        if len(text) > 15 and len(text) <= 30:
            ...
            diff_ratio = (font_size - nfont_size)/float(1280)
        ...
        draw.text((text_x, int(ov_y + ov_h*(text_y_ratio+diff_ratio))), text, text_color, font)
...
#------------------------------ get last frame ----------------------------
    video_duration = float(check_output("ffprobe -v error -show_format " + in_file +
                                        " | grep \"duration\" | awk -F= '/duration/{print $NF}'",
                                        shell=True))
    call("ffmpeg -loglevel 0 -ss " + str(int(video_duration - 0.5)) +
         " -i "+in_file+" -t 1 " + transpose_command2 +
         " -update 1 -f image2 " + last_frame_filename,
         shell=True)
#------------------------------- make new frames ------------------------------
    print "making end frames"
    call('rm -rf '+final_fold, shell=True)
    call('mkdir '+final_fold, shell=True)
    ...
    while i < total_frame_count:
        if not i > fading_frame_count: #make fading frames
            if i < 10:
                ...
                j = "img0"+str(i)
            overlay(last_frame_filename, infold1+"/"+str(i)+".png",
                    infold2+"/"+str(i)+".png", infold3+"/"+str(i)+".png",
                    1100/float(1280), final_fold, j, overlay_ratio)
            ...
            overlay(last_frame_filename, infold1+"/"+str(i)+".png",
                    infold2+"/"+str(i)+".png", infold3+"/"+str(i)+".png",
                    1100/float(1280), final_fold, j, overlay_ratio,
                    white_text, white_text_y_ratio, white_color,
                    text, text_y_ratio, pink_color)
        i = i + 1
#---------------------------------make video from new frames ------------------
    call("ffmpeg -loglevel 0 -i "+final_fold+"/img%03d.jpeg -c:v libx264 " + end_file,
         shell=True)
    call("ffmpeg -loglevel 0 -i "+in_file+" "+transpose_command2+" -qscale:v 1 intermediate1.mpg",
         shell=True)
    call("ffmpeg -loglevel 0 -i "+end_file+" -qscale:v 1 intermediate2.mpg",
         shell=True)
    call("ffmpeg -loglevel 0 -i concat:\"intermediate1.mpg|intermediate2.mpg\" -c copy final.mpg",
         shell=True)

    print "frames added at end of video"
#-------------------------------- delete unnecessary files ----------------------------
    call('rm '+last_frame_filename, shell=True)
    call('rm intermediate1.mpg intermediate2.mpg', shell=True)
    call('rm -rf '+final_fold, shell=True)
    call('rm '+end_file, shell=True)
    return 'final.mpg'

def make_promo(path, in_file, username, transpose_command):
    import os
    promo_content_path = os.path.abspath(os.curdir)+'/promo_content'
    #copy promo_content folder to temp PATH
    shutil.copytree(promo_content_path, path)
    #work in currently copied path
    os.chdir(path)
    #make video
    return promo_video(in_file, text=('/'+username), transpose_command=transpose_command)

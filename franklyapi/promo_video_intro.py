...
def fadeInOriginalVideo():
	...
	while(i<n):
		...
		i=i+1
def makeFinalPromo(answer_author_name,video_file_path,question,question_author_username,answer_author_image_filepath,transpose_command='',temp_path="temprg/vid3",output_file_name="a.mp4"):
	
	path = temp_path

	end_promo_mpg_name = make_promo(path,video_file_path,'kiran.jpg','promo_content/','overlay_png1','overlay_png2','overlay_png3','bariol_bold-webfont_0.ttf',answer_author_name,transpose_command)
	# call('mkdir promo_temp',shell=True)
	# os.chdir("promo_temp")## ????
...
#-------------------------- make video----
	...
	fcall = 'avconv -loglevel 0 -y -i "final2.mpg" -r 25 -vf scale="320:trunc(ow/a/2)*2" -strict experimental -preset veryslow -b:v 256k -pass 1 -c:v libx264  -ar 22050 -ac 1 -ab 44k -f mp4 /dev/null && avconv -y -i "final2.mpg" -r 25 -vf scale="320:trunc(ow/a/2)*2" -strict experimental -preset veryslow -b:v 256k -pass 2 -c:v libx264  -ar 22050 -ac 1 -ab 25k '+final_file

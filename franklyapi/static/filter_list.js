...
(function($) {
	$.fn.btsListFilter = function(inputEl, options) {
		...
		options = $.extend({
			...
			resetOnBlur: false,
			sourceData: null,
			sourceTmpl: '<code class="url"><span>{title}</span></code>',
			...
			itemFilter: function(item, val) {
				...
				console.log(text);
				console.log(regSearch.test( text ));
		...
		inputEl$.on(options.eventKey, debouncer(function(e) {
			...
			if(options.itemChild)
				items$ = items$.find(options.itemChild).filter(function(){
					return $(this).attr('id') === 'search_field';
				});

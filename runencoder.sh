for count in {0..59}
do
	num=`ps -ef|grep python | grep celery|grep encoding|grep celery_encoding.log|wc -l`
	if [ $num -lt 1 ]; then
	...
	num=`ps -ef|grep python | grep celery|grep encoding_low_priority|wc -l`
	if [ $num -lt 1 ]; then
		cd '/home/ubuntu/franklysql/franklyapi'
		echo "$num restarting encoder worker"
		#alertAdmin 
	...
	if [ $num -lt 1 ]; then
		...
		echo "$num restarting encoder worker"
		#alertAdmin 
		/usr/bin/python /usr/local/bin/celery worker --concurrency=2 --autoreload --maxtasksperchild=10 -Ofair  -Q encoding_retry --app=async_encoder -l info -f /tmp/celery_encoding_retry.log --pid /tmp/celery_encoding_retry.pid&
	fi
	sleep 1
done
